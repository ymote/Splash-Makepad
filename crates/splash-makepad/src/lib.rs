...
fn widget_name(kind: NodeKind) -> &'static str {
    match kind {
        ...
        // The six data visualisations. Each is a compiled shader selected by
        // NAME from the DSL — the route `splash-widgets` already documents, and
        // the one §1.1 said had been dismissed too quickly. A DSL node cannot
        // carry shader source, because MPSL compiles at build time; it can name
        // a shader that was compiled.
        NodeKind::TempBar => "L0TempBar",
        NodeKind::SunArc => "L0SunArc",
